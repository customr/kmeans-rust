...
pub fn init_plus_plus<T: KmeansPoint> (kmeans: &mut Kmeans<T>) {
    ...
    for i in 1..kmeans.clusters.capacity() as u8 {
        squared_distance = kmeans.dataset
            .clone()
            .par_iter()
            .map(|p| p.get_nearest_cluster_distance(&kmeans.clusters))
            .collect();

        sum = squared_distance.iter().sum::<f32>();
